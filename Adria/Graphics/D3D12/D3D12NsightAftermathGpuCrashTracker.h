...
namespace adria
{
	...
	class D3D12NsightAftermathGpuCrashTracker
	{
		...
		{
			Bool operator()(GFSDK_Aftermath_ShaderDebugInfoIdentifier const& lhs, GFSDK_Aftermath_ShaderDebugInfoIdentifier const& rhs) const
			{
	...
	public:
		explicit D3D12NsightAftermathGpuCrashTracker(GfxDevice* gfx);
		~D3D12NsightAftermathGpuCrashTracker();
	...
	private:
		...
		{
			ADRIA_TODO("Implement marker resolving");
		}
		...
		{
			D3D12NsightAftermathGpuCrashTracker* gpu_crash_tracker = reinterpret_cast<D3D12NsightAftermathGpuCrashTracker*>(user_data);
